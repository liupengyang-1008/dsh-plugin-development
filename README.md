...
references/       ← 23 篇专题文档（含 6 个上游素材分册），按需读取
scripts/          ← 9 个确定性校验工具
assets/           ← 可直接复制改名的工程骨架
LICENSE / NOTICE / licenses/   ← 许可与第三方声明（零 copyleft）
```
...
技能里的模板是**可复制的骨架**。其中「挂载外部 MCP / 外部进程」的示例会**在 DSH 启动时下载并执行外部包** —— 这是全套模板里唯一有真实安全含义的地方。复制时守住三条：

| # | 约束 | 为什么 |
|---|---|---|
| 1 | **依赖写精确版本**（`pkg@1.2.3`），不要写 `pkg` / `latest` / `^1` / `~1.x` | 浮动规格意味着「每次启动执行的都是当时的线上最新版」。**代码在你审阅之后仍然会变**——上游被攻陷、发布恶意新版本、形近包抢注，都会直接落到本机 |
| 2 | **凭据只列真正需要的那一把，并用低配额、可随时撤销的专用 key** | 那个被下载下来的子进程与 DSH **同权限**运行：能读写文件、能发网络请求，也能读到 `env:` 里列出的**每一个**凭据 |
| 3 | **把「下载」与「执行」分成两步** | 先在独立步骤里显式安装并审阅解析出的依赖树，再让补丁指向本地可执行文件——这样「包里到底跑了什么」在运行前就是确定的 |

> 这三条来自本技能的一次真实安全审计（ClawHub 扫描器报告）。**技能的其它部分不涉及这类风险**：它自身不含运行时逻辑，不读取任何凭据，也不外传数据。
...
| `references/` | **23 篇**专题：主线编号 `00`~`15` + `api-claims`，另 **6 个上游素材分册**（`02b` · `02c` · `10a`~`10d`）。主线为 `00` 版本闸门 · `api-claims` 事实分级 · `01` 心智模型 · `02` 12 个模板 · `03` API 手册 · `04` UI 与插槽 · `05` 23 条坑 · `06` 工作流 · `07` 工程约定 · `08` 速查表 · `09` 结对提示词 · `10` 社区案例库（总索引）· `11` 术语与来源 · `12` 社区插件 · `13` 版本史 · `14` HTTP 与定时器 · `15` 边界与维护 | **按需读取**，不要一次全读 |
| `scripts/` | **9 个**确定性工具：防过时探针（py/sh 双版本）· 第二批断言核验器 · 插槽权威抽取 · 引用一致性守门 · **许可声明自检** · 上游同步 · 版本差异 · 历史矩阵 | 见 [第 5 节](#5-为什么它不承诺不过时) |
| `assets/` | **3 组 8 个文件**，可直接复制：零代码组合包骨架 · 最小工具插件骨架 · 决策留痕模板 | 动手时直接抄 |
| `LICENSE` · `NOTICE` · `licenses/` | 许可与第三方声明：本技能原创内容按 **MIT-0**；引用的第三方片段分别适用 **MIT / Apache-2.0 / BSD-3-Clause**（**零 copyleft**），许可正文三份随包 | 复用或再分发本技能的片段前 |

**大文件用检索、不要整读**（单文件最大 1.9k 行）：
...
**Q：为什么文档里有些数字（23 篇 / 23 条坑 / 56 条断言）看起来像在自我强调？**
...
| 项 | 值 |
|---|---|
| DSH 基线 | tag `dsh-v0.1.6-alpha.1` / commit `0a15e36e7f` / 2026-09-15 |
...
- 探针只覆盖它断言的 56 条事实，**不覆盖 `V` 级实现细节**。探针全绿 ≠ 你的插件能跑。
...
本技能的**原创内容**（散文、工具脚本、模板）以 **MIT-0** 发布 —— 见 [LICENSE](LICENSE)。
