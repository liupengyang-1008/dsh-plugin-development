...
**references 是某一时刻的快照，会腐化。** DSH 处于 rc 阶段（基线 `dsh-v0.1.7-rc.1` / commit `46a7f68b09`，2026-09-23），tag 间隔中位数约 1.1 天、出现过破坏性变更。**所以不要承诺「不过时」，要保证「过时会响」。**
...
**基线 `dsh-v0.1.7-rc.1` 的硬变化（照抄旧资料前先对照）**

DSH 每次发版的破坏性变更集中在少数几类。

**本轮（`0.1.6-alpha.2` → `0.1.7-rc.1`；1617 个提交、4 个新 tag —— 版本链 3 个台阶 `0.1.7-alpha.1` / `-alpha.2` / `-rc.1`，另 `dsh-v0.1.5-rc.3` 为旁线发布、在本区间被后合并进主线）的插件作者必读**：

1. 🔴 **设置机制整套重做 —— 本轮最需要改认知的一条。** 旧路（`ctx.settings.installSection(...)` 注册命名空间 + 浏览器侧手写设置卡片）**已被整体移除**：`installSection`、`SettingsScope`、`settings.yaml`、客户端 `settingsScope` 服务与整个手写卡片族（−4608 / +73 行）**在新树源码里都不存在了**。新路：**表单由你的 `Config` schema 自动派生**（命名空间 = profile 条目 id），持久化改由**配置编辑器写 profile 的 Cordis patch**，写入带乐观并发（`expectedRevision` 不匹配抛 `SettingsConflictError`，`code = 'SETTINGS_CONFLICT'`）。**照抄旧的设置卡片教程会直接失败** → 见 `references/04-ui-and-slots.md` §10 与 §10.9 速查表。
2. **会话格式版本 `3` → `4`**（`SESSION_FORMAT_VERSION`，`packages/core/session/src/types.ts:89`）。已落盘的 session 日志要经迁移链读（新增包 `dsh-session-format-v3-to-v4`）。**别在插件里写死 `3` 这个字面量**，用常量。
3. **插槽移除 1 个：`conversation.session.header.leading`。** 按旧键注册会抛「is not declared」。同族的 `.header` / `.actions` / `.corner` / `.lineage` / `.utilities` 都还在。新增的是**顶层** `conversation.header` / `conversation.header.leading`（都是 single）—— 这是**位置迁移**，新键不在 `conversation.session.*` 命名空间下。
...
1. **插件配置插槽改名 + 换语义**：`settings.plugin.item`（`keyed`，key = 卡片所编辑的命名空间）→ **`plugins.item`**（`list`，用 `id` / `order` / `label`），宿主从 `ui-settings-plugins` 移到新包 `ui-plugin-manager`；同批新增 `plugins.bundle.config` / `plugins.row.config`。**不是别名关系**——旧名注册会抛「is not declared」。
2. **官方包改名**：`@deepseek-ai/cordis-plugin-hmr` → **`@deepseek-ai/dsh-hmr`**（`hmr` 服务键 / `baseDir` / `config` / `hmr/change` / `hmr/reload` **都不变**）。同一 base 行的默认值也改了：`disabled: true` → `!ctx.get('profileContext')`、`root: ['.']` → `root: []`。
3. **CLI 安装实现搬包**：`apps/cli/src/plugin.ts` 由 163 行缩到 25 行，对账逻辑进新包 `dsh-plugin-manager`（`packages/boot/plugin-manager/src/operations.ts`）；「无 `dsh.bundle`」那句警告**删掉了结尾括注**。
4. **客户端产物契约扩展**：banner 从常量改为**按 chunk 生成**的函数，非入口分块多带 `chunk: "<fileName>",` 字段；分块名固定 `client.<name>.js`；包内动态 import 编译成 `require.async()`。**入口 chunk 的 banner 与旧版逐字相同**——只发单个 `client.js` 的插件不受影响。
5. **base 补丁新增** `tool-plugin-manager` / `plugin-manager` 两行。

**上上轮（`dsh-v0.1.5-rc.2` → `0.1.6-alpha.1`，800 个提交）** —— 手上是 rc.2 时代资料的人还要看这六条：
...
**判据**：三条来源都要看 —— 探针（`M33`~`M37` / `N06`~`N08` / `P06`~`P07` / `V01` / `X07` / `U11`~`U12` / `X08`~`X09` 就是这几批）、`dsh-version-diff.sh` 的六维度、以及 `.agents/notes/` 的决策记录。
**⚠️ 别指望 `!:`**：三个区间（800 + 887 + 1617 个提交）里，非 merge 提交带 `!:` 的**分别是 0 / 0 / 1 条**，而最狠的那些改名（`code-runtime` → `ptc-runtime`、`settings.plugin.item` → `plugins.item`）**一条都没标**。本轮那唯一一条标了的，删的还是**旁线上一个从未到达基线的子系统**（activity 平面）。**`!` 既非充分也非必要。**
...
| 用到**入站 HTTP / 定时器 / 客户端产物 / 插槽**任一项 | `verify_absorbed_claims.py <仓库路径>` | 49 条第二批断言。**先跑 `--selftest` 证明它能失败**，再跑正向 |
