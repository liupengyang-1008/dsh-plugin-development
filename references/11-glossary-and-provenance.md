...
**基线 commit**：`46a7f68b0922371ce7144b668b90e377d8e799f4`（2026-09-23）
**版本**：`dsh-v0.1.7-rc.1`
**核查日期**：2026-09-11
...
- ❌ 插件审核 / 白名单
- ❌ 官方仓库接受外部 PR
- ⚠️ `installSection`：**已不适用** —— 该 API 在 `dsh-v0.1.7-rc.1` 被移除（同日移除的还有 `SettingsScope` 与整个手写设置卡片族）。设置机制的权威文档现在是 `docs/subsystems/settings.zh.md`（155 行，本区间被整篇重写；旧版 405 行）
- ⚠️ 面向外部作者的测试脚手架：**未找到**
...
3. **接受它每周都在变。** 作者官方都说 API pre-stable 了。你基于 `dsh-v0.1.7-rc.1` 学到的东西，三个月后可能需要微调。**这不是你学得不好，是这个项目就是这个阶段。**
...
> ⚠️ **最后一条风险提示**：本文件基线是 commit `46a7f68b09`（`dsh-v0.1.7-rc.1`，2026-09-23）。DSH 自称 pre-stable、tag 间隔中位数约 1.1 天且有过破坏性变更。**升级前先跑 `git log --grep='!'` 评估影响**，并把 `package.json` 的 `peerDependencies` 写成**区间**而非钉死某个 rc 号。
