...
**基线 commit**：`0a15e36e7f82b6ed45af6fa9759f29b40dcd965d`（2026-09-15）
**版本**：`dsh-v0.1.6-alpha.1`
**核查日期**：2026-09-11
...
3. **接受它每周都在变。** 作者官方都说 API pre-stable 了。你基于 `dsh-v0.1.6-alpha.1` 学到的东西，三个月后可能需要微调。**这不是你学得不好，是这个项目就是这个阶段。**
...
## A.1 本补充用到的真实仓库（**原列表实为 14 行**，其中 1 个为 AGPL-3.0、已于 2026-09-16 排除 → 现 **13** 个）
...
| **E** | 官方 7 类模板 + 插槽名提取（当时 40 个，⚠️ 非全量 —— 对基线重新抽取为**约 61 个公开键**，见 `api-claims.md` §三） | 1044 | `02b-official-templates.md`（原 `02-templates.md` 附录 E，2026-09-14 拆册） |
...
> （`10a`~`10d`、`02b`、`02c`），正文为**逐行搬迁、未改写**（**⚠️ 例外：`10b-casebook-tools.md` 已于 2026-09-15 / 09-16 因许可整改被定点改写两轮，不再是逐行搬迁**）；`10-community-casebook.md` 转为**总索引**，
...
> ⚠️ **最后一条风险提示**：本文件基线是 commit `0a15e36e7f`（`dsh-v0.1.6-alpha.1`，2026-09-15）。DSH 自称 pre-stable、tag 间隔中位数约 1.1 天且有过破坏性变更。**升级前先跑 `git log --grep='!'` 评估影响**，并把 `package.json` 的 `peerDependencies` 写成**区间**而非钉死某个 rc 号。
