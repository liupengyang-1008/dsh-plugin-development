...
**基线 commit**：`ddefc45fbc7f8e46dd73185e68295696d1297887`（2026-09-17）
**版本**：`dsh-v0.1.6-alpha.2`
**核查日期**：2026-09-11
...
3. **接受它每周都在变。** 作者官方都说 API pre-stable 了。你基于 `dsh-v0.1.6-alpha.2` 学到的东西，三个月后可能需要微调。**这不是你学得不好，是这个项目就是这个阶段。**
...
> ⚠️ **最后一条风险提示**：本文件基线是 commit `ddefc45fbc`（`dsh-v0.1.6-alpha.2`，2026-09-17）。DSH 自称 pre-stable、tag 间隔中位数约 1.1 天且有过破坏性变更。**升级前先跑 `git log --grep='!'` 评估影响**，并把 `package.json` 的 `peerDependencies` 写成**区间**而非钉死某个 rc 号。
